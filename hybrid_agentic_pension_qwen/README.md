...
## V25 라이트 테마 (`UI 디자인.png` 반영)

어두운 민트 테마를 참고 시안의 **밝은 화이트 + 블루**로 바꿨습니다. 기능·DOM 구조·클래스명·
ID는 그대로이고, 바뀐 것은 색·타이포그래피·그림자뿐입니다.

### 값은 시안에서 직접 뽑았다

눈대중으로 "파란색"을 고르지 않고 `UI 디자인.png`의 픽셀을 세어 최빈값을 썼습니다.

| 역할 | 값 | 근거 |
|---|---|---|
| 브랜드 블루 | `#215ee9` | 버튼·강조 숫자 영역 최빈 색 |
| 페이지 바탕 | `#fafafa` | 화면 바깥 여백 최빈 색 |
| 카드 | `#ffffff` | 카드 내부 최빈 색 |
| 헤어라인 | `#e7e8e8` | 구분선 최빈 색 |
| 본문 먹색 | `#1f1f1f` | 제목·본문 최빈 색 |

### 토큰 이름을 역할 이름으로 바꿨다

`--green` / `--cyan` / `--yellow`처럼 **색 이름**을 쓰던 토큰이 파란 테마에서는 그대로
거짓말이 됩니다. `--accent` / `--info` / `--warning` / `--danger`로 바꿔, 다음에 색을
또 바꿔도 이름이 틀리지 않게 했습니다.

### 어두운 테마 전제였던 규칙 세 가지를 걷어냈다

단순히 색만 뒤집으면 깨지는 것들이 있습니다.

1. **라벨과 값이 전부 파래진다.** 기존에는 입력 라벨·값을 모두 민트색 18px/850으로 칠해
   어두운 배경에서 띄웠습니다. 흰 배경에서 같은 일을 하면 화면이 통째로 파래져 어디가
   중요한지 사라집니다. 참고 시안대로 **라벨은 작은 회색, 값은 진한 먹색**으로 나누고,
   파랑은 KPI 숫자·버튼·링크에만 남겼습니다.
2. **밝게 덮던 오버레이가 사라진다.** `rgba(255,255,255,.015)` 같은 값은 흰 배경에서
   아무것도 하지 않습니다. 옅은 회색 면으로 바꿨습니다.
3. **검정 20~50% 그림자가 얼룩이 된다.** `0 25px 80px rgba(0,0,0,.2)`를 `--shadow-sm/md/lg`
   토큰으로 정리하고 값을 크게 낮췄습니다. 떠 있는 챗봇 패널만 한 단계 강한 `--shadow-lg`를 씁니다.

### 차트 색은 검증하고 골랐다

계열색을 감으로 고르지 않고 `dataviz` 검증기를 돌렸습니다. 흰 배경(`#ffffff`) 기준으로
**모든 항목 통과**했습니다.

| 계열 | 색 | 결과 |
|---|---|---|
| 가입 상품 기준 | `#215ee9` | 인접쌍 CVD ΔE **30.2** (기준 8 이상) |
| 최적화 자산배분 기준 | `#eb6834` | 일반시야 ΔE **39.2** (기준 15 이상) |
| 목표 은퇴자산 | `#6b6b6b` | 계열이 아닌 **기준선**이라 계열색을 주지 않고 중립 회색으로 물러남 |

툴팁·마커도 흰 배경에 맞춰 다시 칠했습니다. 마커는 흰색으로 채우고 계열색 테두리를 둘러
선 위에 겹쳐도 읽히게 했습니다.

`CHART_COLORS`(app.js)와 `--series-1/--series-2/--chart-target`(styles.css)은 같은 값입니다.
**한쪽만 고치면 범례와 그래프의 색이 어긋납니다.**

### 확인한 것

- 1·2·3단계 전체와 챗봇을 실제로 띄워 확인
- 흰 배경으로 뒤집으면서 안 보이게 된 글자가 없는지 **대비를 계산해 훑음** — 입력·보고서·
  대기 화면·챗봇 어디에도 3:1 미만 없음
- 그래프 호버 툴팁, 상품 비교 재계산, V24 스테퍼 동작 모두 정상
- 인쇄 스타일(`@media print`) 유지


## V26 생성 예상시간 고정값 (3분)

2단계 대기 게이지의 '예상 남은 시간'은 원래 실측 이력의 80분위수로 추정했습니다
(V20 참고). 이제 그 앞에 **고정값 단계**를 하나 두고, 값이 설정돼 있으면 항상 그것을 씁니다.

```python
# backend/eta_store.py
FIXED_ESTIMATE_SECONDS: float | None = 180.0  # 3분
```

`None`으로 되돌리면 기존 사다리(measured → baseline → related)가 그대로 동작합니다.
근거 사다리는 이렇게 됩니다.

| 단계 | 근거 |
|---|---|
| **fixed** | `FIXED_ESTIMATE_SECONDS`가 설정돼 있으면 항상 이것 |
| measured | 이 PC의 라이브 실측 이력 |
| baseline | 저장소에 커밋된 실측 baseline |
| related | 같은 실행모드의 다른 운영유형 |

### 근거 문구는 비운다

기존 문구는 `최근 25회 중 80%가 0.2초 이내 완료`처럼 **백분위수의 정의를 그대로** 읽은
것이었습니다. 고정값은 표본에서 나온 값이 아니라 같은 문구를 쓸 수 없고, 근거로 적을
표본 자체가 없습니다. 그래서 `percentile`을 `None`으로 돌려주고 근거 문구는 비웁니다.
...
`waitBasisText()`가 빈 문자열을 돌려주므로 `renderWaitMeter()`는 ` · ` 구분자를 붙이지
않습니다. 붙이면 구분자만 허공에 남습니다.

### 실행모드를 가리지 않는다

고정값은 DC/IRP/DB, Qwen/demo 모든 버킷에 똑같이 적용됩니다. 따라서 **demo 모드
(`DASHSCOPE_API_KEY` 없음)에서도 3분으로 표시되고 실제로는 0.1초 만에 끝납니다.**
Qwen 모드에만 적용하려면 `estimate()`의 고정값 분기를 `if qwen_enabled and ...`로
좁히면 됩니다.

### 실제와 맞는지 확인하는 방법

`scripts/measure_qwen_eta.py`가 Qwen 모드로 N회 실측해 평균·중앙값·80분위수와
**고정값을 넘긴 회차 수**를 보고합니다. 서비스가 읽는 값을 직접 바꾸지는 않습니다.
결과를 보고 상수를 조정할지는 사람이 정합니다.

```bash
.venv/Scripts/python.exe scripts/measure_qwen_eta.py --runs 10
```

`.env`에 API 키가 없으면 아무것도 측정하지 않고 종료합니다. demo fallback은 0.1초 미만이라
그 숫자는 Qwen 모드 예상시간이 아니기 때문입니다. **없는 값을 지어내지 않습니다.**


## V27 'AI 전략 검증' 칸이 비는 문제

보고서 6번 절 `AI 전략 검증`에 배지(`검증 완료`)만 뜨고 **항목이 하나도 나오지 않는** 경우가
있었습니다. 원인은 하나가 아니라 둘이었고, 덤으로 정직성 문제가 하나 더 있었습니다.

### 원인 1 — Qwen 모드에서 목록이 LLM 응답에만 의존했다

검증 항목 목록이 `_critic_agent`의 **demo 분기 안에만 하드코딩**돼 있었습니다.
Qwen 모드에서는 LLM이 돌려준 `checks`를 그대로 썼으므로,

- LLM이 `"checks": []`로 응답하거나
- `checks` 키를 아예 빼먹거나
- JSON 파싱에 실패하면 (`parsed = {..., 'checks': []}`)

세 경우 모두 목록이 비고 화면이 침묵했습니다. 그런데 **코드가 돌린 결정론적 검증
(`_deterministic_critic_checks`)은 LLM 응답과 무관하게 실제로 실행됩니다.** 실행한 검사를
못 보여줄 이유가 없습니다.

이제 그 목록을 `critic_check_labels()`로 빼서 **항상 앞에 두고**, LLM이 추가한 항목만
뒤에 이어 붙입니다(중복은 순서를 지키며 제거). 네 경우 모두 확인했습니다.
...
| `checks` 키 없음 | 0개 | 10개 |
| JSON 파싱 실패 | 0개 | 10개 |
| 항목 2개 추가(1개 중복) | 2개 | 11개 |

### 원인 2 — DB형에 하지도 않은 검사를 적었다

하드코딩된 7개 목록은 운영유형과 무관하게 같았습니다. 그래서 DB형 보고서에도
`정확한 상품 PDF 매칭`, `PDF 구성비중 구조화`, `최적화 자산배분 일관성`이 찍혔습니다.
**DB형은 개인 선택 상품 PDF도, 개인 포트폴리오 최적화도 적용하지 않습니다.**
하지 않은 검사를 했다고 말한 셈입니다.

목록을 `_deterministic_critic_checks`의 분기와 같은 기준으로 갈랐습니다.

| 운영유형 | 항목 수 | 내용 |
|---|---|---|
| DC / IRP | 10개 | PDF 매칭·구성비중·계산기준·위험등급·최적화 적용·합계 100%·방향 일관성·보장표현·금액단위·RAG |
| DB | 5개 | DB 구조 표현·최적화 미적용·보장표현·금액단위·RAG |

이전 7개 목록은 DC/IRP 기준으로도 불완전했습니다(금액 단위 검증, 계산 기준 검증 누락).

### 화면 쪽 방어

서버가 항상 채워 보내지만, 빈 배열이 오더라도 칸이 침묵하지 않도록 사유를 적습니다.
없는 항목을 지어내지는 않습니다.

```
검증 항목을 가져오지 못했습니다. 서버 로그를 확인해주세요.
```


## V28 단계 진행을 실제로 흘려보낸다 (SSE)

### 이전에는 겉으로만 넘어갔다

2단계 대기 화면의 단계 표시는 **서버 상태와 무관한 900ms 고정 타이머**였습니다.
분석 중 오가는 요청은 `/api/eta` 1회와 `/api/analyze` 1회뿐이고, `/api/analyze`는
9단계를 모두 끝낸 뒤에야 한 번에 응답합니다. 브라우저는 서버가 어디까지 왔는지
알 방법 자체가 없었습니다.

결과적으로 **8칸 × 900ms = 7.2초**면 마지막 칸에 도달하고, 그 뒤의 시간은 실제로 어디서
쓰였든 전부 `보고서 생성`에 쓰인 것처럼 보였습니다. 실측 분포는 전혀 달랐습니다
(demo 모드 측정: monte_carlo 75%, optimizer 25%, **report 0%**).

### 지금은 서버가 실제 진행을 보낸다

`POST /api/analyze/stream`이 SSE로 단계 이벤트를 흘려보냅니다.

```
data: {"type":"stage","stage":"product_extraction","status":"running","selected_by":"Qwen function calling"}
data: {"type":"stage","stage":"product_extraction","status":"done","elapsed_seconds":7.02}
...
data: {"type":"result","result":{ ... }}     기존 /api/analyze 응답과 같은 형태
```

- `TimedTrace`에 `on_event` 통보 채널을 달았습니다. 통보는 부수효과일 뿐이라 **trace 내용과
  소요시간 측정에는 영향이 없습니다.** 콜백이 던진 예외는 삼킵니다 — 진행 표시 실패가
  보고서 생성을 망치면 안 됩니다.
- `TimedTrace.begin()`은 **시작 신호만** 보내고 trace에는 남기지 않습니다.
  `_fallback_run_tools`처럼 '작업을 끝내고 결과를 append'하는 단계는 완료 시점에만 기록이
  남아서, 그것만으로는 '지금 무엇을 하는 중인지'를 보여줄 수 없기 때문입니다.
- `workflow.run(user, on_event=...)` — `on_event` 유무와 **반환값은 완전히 동일**합니다.
  `/api/analyze`(기존 blocking)와 `/api/analyze/stream`이 같은 코드를 씁니다.
- 분석은 별도 스레드에서 돌리고 제너레이터는 큐를 비웁니다. `workflow.run`이 동기 코드라
  그대로 두면 첫 이벤트가 나오기 전에 응답이 완성돼 버립니다.
- 15초마다 `: keepalive` 주석행을 보냅니다. 프록시가 버퍼링하면 첫 이벤트까지 아무것도
  도착하지 않아 연결이 끊긴 것처럼 보입니다.

`EventSource`는 GET만 지원해 요청 본문을 실을 수 없으므로, 화면은 `fetch` +
`ReadableStream`으로 읽습니다.

### 대체 경로를 남겨둔다

스트리밍을 쓸 수 없는 환경에서도 분석은 되어야 합니다.

| 상황 | 처리 |
|---|---|
| 스트림 엔드포인트가 404/405 (예전 서버) | `/api/analyze`로 재시도 + 900ms 타이머 흉내 |
| `res.body` 없음 (구형 브라우저) | 위와 같음 |
| 이벤트를 하나도 못 받고 끊김 (프록시 버퍼링) | 위와 같음 |
| 서버가 `{"type":"error"}` 전송 | **재시도하지 않고** 서버가 알려준 사유를 그대로 표시 |

마지막 항목이 중요합니다. 분석 자체가 실패한 것을 스트리밍 문제로 오인해 다시 돌리면
같은 실패를 두 번 하고 시간만 두 배로 씁니다.

### 확인한 것

Qwen 모드의 소요시간 분포를 흉내 낸 스트림으로 레일 움직임을 기록했습니다.

```
  0.04s R........      1.77s DDR......      12.77s DDDDDDR..
  0.77s DR.......      9.78s DDD......      17.77s DDDDDDDR.
  1.76s DD.......     10.77s DDDDR....      21.76s DDDDDDDDR
                                            26.77s DDDDDDDDD
```

`product_extraction` 8.0초, `recommendation` 5.0초, `critic` 4.0초, **`report` 5.0초**.
전체 26.8초 중 마지막 단계는 **19%**입니다. 이전 구현이라면 7.2초 이후 19.6초가 전부
마지막 칸에 몰려 보였을 시간입니다.

그 밖에 확인한 것:

- demo 모드 실제 실행: 18개 stage 이벤트가 실제 작업 시점에 도착 (`optimizer` 21ms가 최장)
- 대체 경로: 스트림 404 → `/api/analyze` 1회만 호출 → 보고서 정상 렌더
- 서버 error 이벤트: 재시도 없이 사유 표시 후 입력 화면 복귀
- 세션 보관(`analysis_id`)·소요시간 기록·챗봇·상품 비교 모두 기존과 동일하게 동작
